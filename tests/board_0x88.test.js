...
describe('0x88 Board Representation', () => {
  ...
  test('Board should use a 1D array of size 128', () => {
    // We expect the internal storage to be 'squares' or similar,
    // but for now let's assume we rename 'grid' or create a new property.
    // Based on plan, we will use 'squares'.
    expect(board.squares).toBeDefined();
    expect(board.squares.length).toBe(128);
  });
  ...
  test('Board setup should place pieces in correct 0x88 indices', () => {
    ...
    expect(board.squares[0x70]).not.toBeNull();
    expect(board.squares[0x70].type).toBe('rook');
    expect(board.squares[0x70].color).toBe('white');

    expect(board.squares[0x77]).not.toBeNull();
    expect(board.squares[0x77].type).toBe('rook');

    // Black Rooks at 0,0 (0x00) and 0,7 (0x07)
    expect(board.squares[0x00]).not.toBeNull();
    expect(board.squares[0x00].type).toBe('rook');
    expect(board.squares[0x00].color).toBe('black');
  });
});
